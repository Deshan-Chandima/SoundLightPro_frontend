...
const OrderManager = ({
    ...
    const handleCreateOrder = async (status = 'Confirmed') => {
        ...
        // ID will be generated by the backend in YYMONNNNNN format (e.g. 26APR0001)
        const newOrder = {
            id: 'PENDING',
        ...
        if (sqlMode) {
            try {
                ...
                // Backend returns the saved order WITH the generated sequential ID
                const savedOrder = await api.saveOrder(newOrder);
                const orderWithId = savedOrder && savedOrder.id ? savedOrder : newOrder;
    ...
    const handleUpdateOrderSubmit = async (e) => {
        ...
        try {
            ...
            const updatedOrder = {
                ...
                damageFee,
                eventName: updateFormData.eventName !== undefined ? updateFormData.eventName : updatingOrder.eventName,
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-1 space-y-6">
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Event Name (Optional)</label>
                                    <input
                                        type="text"
                                        placeholder="e.g. Wedding, Concert, Corporate Event..."
                        ...
                        <form onSubmit={handleUpdateOrderSubmit} className="p-4 sm:p-6 space-y-6 overflow-y-auto">
                            <div className="bg-indigo-50 p-4 rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4 shrink-0">
                                ...
                                <div className="col-span-1 sm:col-span-2">
                                    <label className="block text-xs font-bold text-indigo-400 uppercase tracking-widest mb-1">Event Name (Optional)</label>
                                    <input
                                        type="text"
                                        className="w-full px-3 py-2 bg-white/50 border border-indigo-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 text-sm font-bold text-indigo-900"
                                        placeholder="Add or edit event name..."
                                        value={updateFormData.eventName || ''}
                                        onChange={(e) => setUpdateFormData({ ...updateFormData, eventName: e.target.value })}
                                    />
                                </div>
                            </div>
